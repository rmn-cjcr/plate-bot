...
URL = "https://999.md/ru/list/transport/cars?applied=1&o_1_21_34=22562&o_1_21_34=78&o_4_151=10&r_6_2_unit=eur&ef=260" \
...
class Scrapper:
    ...
    def _get_pages_count(self):
        response = requests.get(URL.replace(':page', '9999'))
    ...
    def find_images(self):
        ...
        for n in range(self._pages_counter):
            response = requests.get(URL.replace(':page', str(n+1)))
            soup = BeautifulSoup(response.content, 'html.parser')
            item_list = soup.find_all('a', class_='js-item-ad')
            for item in item_list:
                if item.has_attr('href') and item.find('img') is not None:
                    self.ads_list.append(CarItem(item.find('img').get('src'), item.get('href')))
            logging.info(f'Pages processed {n+1}')
